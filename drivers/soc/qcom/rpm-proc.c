...
static int rpm_proc_probe(struct platform_device *pdev)
{
	...
	/*
	 * The RPM firmware is usually already started by earlier firmware.
	 * However, on some firmware versions the RPM firmware is only loaded
	 * but not brought out of reset because Linux needs to do some
	 * initialization first (e.g. set up the IOMMU).
	 */
	...
	if (ret) {
		dev_err(dev, "Failed to release RPM from reset: %d\n", ret);
		goto err;
	}
